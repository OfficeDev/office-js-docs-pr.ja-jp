...
1. 1 行目で、次のコードを一番上に挿入します。 これにより、**sharedState** という名前のグローバル変数が初期化されます。

    ```js
    window.sharedState = "empty";
    ```

1. 次のコードを追加して、値を **sharedState** 変数に保存するカスタム関数を作成します。
...
1. 次のコードを追加して、**sharedState** 変数の現在の値を取得するカスタム関数を作成します。
...
1. 次のスクリプト要素を追加してから、`</head>` 要素を閉じます。

    ```HTML
    <script src="../functions/functions.js"></script>
    ```

1. 終了 `</main>` 要素の後に、次の HTML を追加します。 HTML は、グローバル データの取得または保存に使用される 2 つのテキスト ボックスとボタンを作成します。
...
1. `</body>` 要素を閉じる前に、次のスクリプトを追加します。このコードは、ユーザーがグローバル データを保存または取得するときにボタンのクリック イベントを処理します。
