...
|**OfficeTab**|既定のOffice アプリリボン タブを拡張する場合は必須です (**プライマリ コマンドサーフェス** を使用)。 **OfficeTab** 要素を使用する場合は、**カスタムタブ** 要素を使用できません。 詳細については、「[OfficeTab](officetab.md)」を参照してください。|
...
|**Label**|必須。 グループのラベルです。 **resid** 属性は 32 文字以内で指定でき **、String** 要素の **id** 属性の値に設定する必要があります。 **String** 要素は、 **Resources** 要素の子要素である **ShortStrings** 要素の子要素です。|
|**Icon**|必須。 小さいフォーム ファクターのデバイス、または表示されるボタンが多すぎるときに使用されるグループのアイコンを指定します。 **resid** 属性は 32 文字以内で **、Image** 要素の **id** 属性の値に設定する必要があります。 **Image** 要素は、 **Resources** 要素の子要素である **Images** 要素の子要素です。 **size** 属性は、イメージのサイズをピクセル単位で指定します。 3 つのイメージのサイズ (16、32、80) が必要です。 5 つのオプションのサイズ (20、24、40、48、64) もサポートされています。|
|**Tooltip**|省略可能。 グループのツールヒント。 **resid** 属性は 32 文字以内で指定でき **、String** 要素の **id** 属性の値に設定する必要があります。 **String** 要素は、 **Resources** 要素の子要素である **LongStrings** 要素の子要素です。|
|**Control**|各グループには、少なくとも 1 つのコントロールが必要です。 **コントロール** 要素は、**ボタン** または **メニュー** のいずれかです。 **[メニュー] を** 使用して、ボタン コントロールのドロップダウン リストを指定します。 現在は、ボタンとメニューのみがサポートされています。 詳細については、「[Button コントロール](control.md#button-control)」および「[Menu コントロール](control.md#menu-dropdown-button-controls)」のセクションを参照してください。<br/>**注:**  トラブルシューティングを容易にするために **、Control** 要素と関連する **リソース** の子要素を一度に 1 つずつ追加することをお勧めします。|
...
- [コマンドサーフェス](#mobileonlinemeetingcommandsurface)
...
> [!IMPORTANT]
> [メールボックス](../objectmodel/preview-requirement-set/office.context.mailbox.md#events)イベントと[アイテム](../objectmodel/preview-requirement-set/office.context.mailbox.item.md#events)イベントの登録は、この拡張ポイントでは使用できません。
...
### <a name="mobileonlinemeetingcommandsurface"></a>コマンドサーフェス

この拡張ポイントは、モバイル フォーム ファクターの予定のコマンド画面に、モードに適したトグルを配置します。 会議の開催者は、オンライン会議を作成できます。 出席者は、その後オンライン会議に参加できます。 このシナリオの詳細については、「[オンライン会議プロバイダー用のOutlookモバイル アドインを作成する](../../outlook/online-meeting.md)」を参照してください。

> [!NOTE]
> この拡張ポイントは、Microsoft 365サブスクリプションを持つ Android および iOS でのみサポートされます。
>
> [メールボックス](../objectmodel/preview-requirement-set/office.context.mailbox.md#events)イベントと[アイテム](../objectmodel/preview-requirement-set/office.context.mailbox.item.md#events)イベントの登録は、この拡張ポイントでは使用できません。
...
`ExtensionPoint` この型の要素は、1 つの子要素 (要素) を持つことができます `Control` 。

`Control`この拡張ポイントに含まれる要素は、属性が に設定されている必要があります `xsi:type` `MobileButton` 。

`Icon`画像は、16 進コード `#919191` または他の[カラー形式](https://convertingcolors.com/hex-color-919191.html)で同等のグレースケールにする必要があります。
...
### <a name="launchevent-preview"></a>イベントの起動 (プレビュー)

> [!NOTE]
> この拡張ポイントは、web 上のOutlookで[プレビュー](../objectmodel/preview-requirement-set/outlook-requirement-set-preview.md)で、Microsoft 365サブスクリプションでWindowsでのみサポートされます。

この拡張ポイントを使用すると、デスクトップ フォーム ファクターでサポートされているイベントに基づいてアドインをアクティブ化できます。 このシナリオの詳細と、サポートされているイベントの完全な一覧については、「[イベント ベースのアクティブ化用にOutlook アドインを構成する](../../outlook/autolaunch.md)」を参照してください。
...
|  要素 |  説明  |
|:-----|:-----|
| [LaunchEvents](launchevents.md) |  イベント ベースのアクティブ化の [起動イベント](launchevent.md) の一覧。  |
...
この拡張点は、指定したイベントのイベント ハンドラーを追加します。 この拡張ポイントの使用の詳細については[、「Outlook アドインの送信機能](../../outlook/outlook-on-send-addins.md)」を参照してください。
...
必ず指定します。 グループのラベルです。 **レジ情報** 属性は 32 文字以内で、[リソース](resources.md)要素の **ShortStrings** 要素の **文字列** 要素の **id** 属性の値に設定する必要があります。
