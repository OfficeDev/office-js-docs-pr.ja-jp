...
|要件| 値|
|---|---|
|[メールボックスの要件セットの最小バージョン](/javascript/office/requirement-sets/outlook-api-requirement-sets)| 1.0|
...
文字列は、 `Outlook`、`Mac Outlook`、`OutlookIOS`、または `OutlookWebApp` のいずれかの値になります。
...
*   `OneColumn`画面幅が狭い場合に表示される 。Outlook Web App は、この単一列レイアウトを使用してスマートフォンの画面全体への表示を行います。
