...
次の表に、Excel JavaScript API 要件セット1.11 の Api を示します。 Excel JavaScript API 要件セット1.11 またはそれ以前でサポートされているすべての Api の API リファレンスドキュメントを表示するには、「[要件セット1.10 またはそれ以前の Excel api](/javascript/api/excel?view=excel-js-1.11)」を参照してください。
...
||[id](/javascript/api/excel/excel.commentmention#id)|エンティティの id。 Id は、のいずれかの id `CommentRichContent.richContent`と一致します。|
