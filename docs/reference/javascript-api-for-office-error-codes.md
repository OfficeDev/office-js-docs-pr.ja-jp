...
|3010|サポートされないバインド操作|選択するコンテンツはテーブル形式にする必要があります。 データをテーブルとして書式設定して、もう一度やり直してください。|開発者は、オブジェクトの or メソッドを、coercion 型マトリックスのデータ `addRowsAsync` `deleteAllDataValuesAsync` `TableBinding` に使用 _しようとしている_。|
...
|5004|無効な API 呼び出し|現在のコンテキストで無効な API 呼び出しです。|コンテキストに対して無効な呼び出しが行われた (たとえば、コンテキスト内でオブジェクトを使用 `CustomXMLPart` Excel。|
...
|9043|添付ファイルの種類 がサポートされていません|添付ファイルの種類はサポートされていません。|API は添付ファイルの種類をサポートしています。 たとえば、添付ファイルがリッチ テキスト形式の埋め込み画像である場合、またはメールや予定表アイテム以外のアイテムの種類 (連絡先やタスク アイテムなど) の場合は、このエラーを `item.getAttachmentContentAsync` スローします。|
