...
この記事では、Common API モデルの使用中に発生する可能性があるエラー メッセージについて説明します。 これらのエラー コードは、JavaScript API や Word JavaScript API などのアプリケーション固有の API には適用Excel適用されません。

共通 [API とアプリケーション](../develop/understanding-the-javascript-api-for-office.md#api-models) 固有の API モデルの違いについては、「API モデル」を参照してください。
...
|:-----|:-----|:-----|:-----|
|1000|無効な強制型変換|指定された強制型変換はサポートされていません。|coercion の種類は、アプリケーションではOfficeされません。 (たとえば、OOXML 型と HTML 型の coercion 型は、この型ではExcel)。|
...
|1008|データの読み取りエラー|要求されたデータ セットが大きすぎます。|ユーザーは、アプリケーションによって定義されたデータ制限を超えてデータを読み取Officeします。|
...
|2009|データの書き込みエラー|指定されたデータ オブジェクトが大きすぎます。|ユーザーは、アプリケーションによって定義されたデータ制限を超えてデータOfficeします。|
...
|3010|サポートされないバインド操作|選択するコンテンツはテーブル形式にする必要があります。 データをテーブルとして書式設定して、もう一度やり直してください。|開発者は、オブジェクトの or `addRowsAsync` `deleteAllDataValuesAsync` `TableBinding` メソッドを、coercion 型マトリックスのデータで使用 _しようとしている_。|
...
|5000|古い設定のエラー|この操作はサポートされていません。|この操作は、現在のアプリケーションではOfficeされません。 たとえば、ユーザー`document.getSelectionAsync`から呼び出Outlook。|
...
|5004|無効な API 呼び出し|現在のコンテキストで無効な API 呼び出しです。|コンテキストに対して無効な呼び`CustomXMLPart`出しが行われた (たとえば、コンテキスト内でオブジェクトを使用Excel。|
...
|5012|無効またはタイム アウトになるセッション|ブラウザー Officeの有効期限が切れているか、無効です。 続行するには、ページを更新します。|Office クライアントとサーバー間のセッションの有効期限が切れたか、お使いのコンピューターで、日付、時刻、タイム ゾーンが正しくありません。|
|6000|無効なノード|指定されたノードが見つかりませんでした。|ノード `CustomXmlPart` が見つかりませんでした。|
...
|9020|汎用応答エラー|内部エラーが発生しました。|内部エラー状態を参照します。これは、任意の数の理由で発生する可能性があります。|
|9021|エラーの保存|サーバーにアイテムを保存しようとして接続エラーが発生しました。|アイテムを保存できません。 これは、Outlook デスクトップでオンライン モードを使用している場合、または Exchange サーバーから削除された下書きアイテムを再保存しようとした場合に、サーバー接続エラーが発生した可能性があります。|
|9022|異なるストアエラーのメッセージ|メッセージが別のストアに保存されたため、EWS ID を取得できません。|メッセージが移動された可能性がある、または送信メールボックスが変更された可能性がある場合、現在のメッセージの EWS ID を取得できませんでした。|
|9041|ネットワーク エラー|ユーザーはネットワークに接続されていません。 ネットワーク接続を確認し、やり直してください。|ユーザーがネットワークまたはインターネット にアクセスできなくなりました。|
|9043|添付ファイルの種類 がサポートされていません|添付ファイルの種類はサポートされていません。|API は添付ファイルの種類をサポートしています。 たとえば、 `item.getAttachmentContentAsync` 添付ファイルがリッチ テキスト形式の埋め込み画像である場合、またはメールや予定表アイテム以外のアイテムの種類 (連絡先やタスク アイテムなど) の場合は、このエラーをスローします。|
...
- [JavaScript API のExcel処理](../excel/excel-add-ins-error-handling.md)
