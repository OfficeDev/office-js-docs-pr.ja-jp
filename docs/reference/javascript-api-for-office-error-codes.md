...
ms.locfileid: "49870666"
---
# <a name="office-common-api-error-codes"></a>Office API の一般的なエラー コード

この記事では、共通 API モデルの使用中に発生する可能性があるエラー メッセージについて説明します。 これらのエラー コードは、Excel JavaScript API や Word JavaScript API などのアプリケーション固有の API には適用されません。

共通 [API と](../develop/understanding-the-javascript-api-for-office.md#api-models) アプリケーション固有の API モデルの違いについて詳しくは、API モデルをご覧ください。 
...
|:-----|:-----|:-----|:-----|
|1000|無効な強制型変換|指定された強制型変換はサポートされていません。|この型の型の型は、新しいアプリケーションOfficeされていません。 (たとえば、OOXML 型と HTML 型の型変換は Excel ではサポートされていません)。|
...
|1008|データの読み取りエラー|要求されたデータ セットが大きすぎます。|ユーザーは、アプリケーションで定義されたデータ制限を超えてデータを読み取Officeします。|
...
|2009|データの書き込みエラー|指定されたデータ オブジェクトが大きすぎます。|ユーザーは、アプリケーションで定義されているデータ制限を超えてデータOfficeします。|
...
|3010|サポートされないバインド操作|選択するコンテンツはテーブル形式にする必要があります。 データをテーブルとして書式設定して、もう一度やり直してください。|開発者は、オブジェクトの or メソッドを型変換型マトリックスのデータ `addRowsAsync` `deleteAllDataValuesAsync` `TableBinding` で使用 _しています_。|
...
|5000|古い設定のエラー|この操作はサポートされていません。|この操作は、現在のアプリケーションではOfficeされていません。 たとえば `document.getSelectionAsync` 、Outlook から呼び出されます。|
...
|5004|無効な API 呼び出し|現在のコンテキストで無効な API 呼び出しです。|Excel でオブジェクトを使用しようとして、コンテキストに対して無効な呼び `CustomXMLPart` 出しが行われた。|
...
|5012|無効またはタイム アウトになるセッション|ブラウザー Office有効期限が切れているか、無効です。 続行するには、ページを更新します。|Office クライアントとサーバー間のセッションの有効期限が切れたか、お使いのコンピューターで、日付、時刻、タイム ゾーンが正しくありません。|
...
|9021|保存エラー|サーバーにアイテムを保存しようとして接続エラーが発生しました。|アイテムを保存できません。 オンライン モードでは、サーバー接続エラーが原因である可能性があります。|
