...
|3010|サポートされないバインド操作|選択するコンテンツはテーブル形式にする必要があります。 データをテーブルとして書式設定して、もう一度やり直してください。|開発者が、強制型変換`addRowsAsynch`の`deleteAllDataValuesAsynch`データに対し`TableBinding`て、オブジェクトのメソッドを使用しようとし_ています_。|
...
|5004|無効な API 呼び出し|現在のコンテキストで無効な API 呼び出しです。|Excel でオブジェクトを`CustomXMLPart`使用しようとするなど、コンテキストに対して無効な呼び出しが行われました。|
...
|9043|サポートされていない添付ファイルの種類|添付ファイルの種類はサポートされていません。|この API は、添付ファイルの種類をサポートしていません。 たとえば、添付`item.getAttachmentContentAsync`ファイルがリッチテキスト形式の埋め込み画像である場合、または電子メールまたは予定表アイテム (連絡先や仕事アイテムなど) 以外のアイテムタイプである場合に、このエラーをスローします。|
