...
> [!NOTE]
> Web 上の Outlook では、Outlook browser タブ内で構成されているメッセージで送信時機能がトリガーされると、そのアイテムは、検証およびその他の処理を完了するために、独自のブラウザーウィンドウまたはタブにポップアウトされます。
...
- 本文を呼び出す場合は、**追加-送信** 機能 &ndash; [。AppendOnSendAsync](/javascript/api/outlook/office.body?view=outlook-js-1.9&preserve-view=true#appendonsendasync-data--options--callback-) on 送信ハンドラーでは、エラーが返されます。
...
|ポリシーの状態|結果|
|---|---|
|無効|送信中のメッセージまたは会議アイテムに対して、オンプレメールアドインの現在ダウンロードされているマニフェスト (最新バージョンではない場合があります) を実行します。 これは既定の状態/動作です。|
|Enabled|オンプレメールアドインの最新のマニフェストが Exchange からダウンロードされると、そのアドインはメッセージまたは会議アイテムが送信されるときに実行されます。 それ以外の場合、send はブロックされます。|
...
|キーの状態|結果|
|---|---|
|false|送信中のメッセージまたは会議アイテムに対して、オンプレメールアドインの現在ダウンロードされているマニフェスト (最新バージョンではない場合があります) を実行します。 これは既定の状態/動作です。|
|true|オンプレメールアドインの最新のマニフェストが Exchange からダウンロードされると、そのアドインはメッセージまたは会議アイテムが送信されるときに実行されます。 それ以外の場合、send はブロックされ、[ **送信** ] ボタンは無効になります。|
