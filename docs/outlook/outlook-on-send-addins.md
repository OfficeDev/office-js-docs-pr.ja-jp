...
検証は、送信イベントのトリガー時に、Outlook のクライアント側で行われます。 検証に失敗すると、アイテムの送信がブロックされ、ユーザーに対応するように求めるエラー メッセージが情報バーに表示されます。  
...
- `NoSend` &ndash; 通知メッセージを参照するための開発者が指定したキーである文字列。 これを使用して後でこのメッセージを変更できます。 キーは 32 文字以内する必要があります。
