...
- または ItemHasRegularExpressionMatch ルールで正規表現を使用する場合 `ItemHasKnownEntity` は、通常、すべての Outlook ホストに適用される以下の制限とガイドラインに注意してください。また、ホストに応じて異なる表1、2、3で説明します。 [ItemHasRegularExpressionMatch](../reference/manifest/rule.md#itemhasregularexpressionmatch-rule)
