...
Office アドイン プラットフォームを使用すると、Office アプリケーションを拡張して、Office ドキュメント内のコンテンツを対話方式で操作するソリューションを構築できます。Office アドインでは、HTML、CSS、および JavaScript などの一般的な Web テクノロジを使用することで、Outlook、Excel、Word、PowerPoint、OneNote、および Project を拡張して対話できるようになります。Windows、Mac、iPad、およびブラウザーを含む複数のプラットフォームにわたって Office ソリューションを実行できます。
...
- **Office クライアントに新しい機能を追加する** - Office に外部データを取り込む、Office ドキュメントを自動化する、Microsoftやその他の Office クライアントの機能を公開する、などがあります。たとえば、Microsoft Graph API を使用して、生産性の向上につながるデータに接続します。
