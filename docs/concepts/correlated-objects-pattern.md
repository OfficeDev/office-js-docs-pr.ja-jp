...
> [!NOTE]
> この記事では、 &mdash; バッチシステムを使用して Office ドキュメントを操作する、Excel、Word、OneNote、Visio 用の4つのホスト固有の Office JavaScript api のうち、少なくとも1つを操作する最初の段階にとどまらないことを前提としてい &mdash; ます。 特に、呼び出しとは何か、コレクションオブジェクトについて理解しておく必要があることを理解しておく必要があり `context.sync` ます。 その段階になっていない場合は、「 [Office JAVASCRIPT API](../develop/understanding-the-javascript-api-for-office.md) 」と「ホスト固有」の下にリンクされているドキュメントを理解してください。
