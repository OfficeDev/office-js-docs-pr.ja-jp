...
- モーダルな操作は許可されていません`alert`。 `confirm`たとえば、 `prompt` JavaScript、、および関数の呼び出しは、モーダルであるため許可されていません。
...
- ユーザーは、AppSource へのアクセスを有効または無効にすることができます。 コンテンツアドインと作業ウィンドウアドインの場合、ユーザーは、ホスト Office クライアントの**セキュリティセンター**から信頼されたアドインとカタログへのアクセスを管理します ([**ファイル** > **オプション]** > **信頼** > センターのセキュリティセンターの**設定** > ] [信頼**できるアドインカタログ**] から開きます)。 Outlook アドインでは、を使用してアドインを管理するには、[アドインの**管理**] ボタンを選択します。 Windows の outlook の場合は、[**ファイル** > の**管理**] [アドイン] を選択します。Outlook on the Mac で、アドインバーの [アドインの**管理**] ボタンをクリックします。 Outlook on the web では、**[設定]** メニュー (歯車アイコン) > **[アドインを管理する]** を選択します。管理者は[グループ ポリシーを使用することによって](/previous-versions/office/office-2013-resource-kit/jj219429(v=office.15)#using-group-policy-to-manage-how-users-can-install-and-use-apps-for-office)、このようなアクセスも管理することができます。
