...
*図 2.Mac OS X クライアントの Office アドイン ランタイム環境*

![Mac OS X 上の Office ランタイム環境用のアプリを示す図。](../images/dk2-agave-overview-mac-02.png)
...
Outlook 固有の動作の詳細については、「 [Outlook アドインのプライバシー、アクセス許可、およびセキュリティ](../outlook/privacy-and-security.md#optional-connected-experiences)」を参照してください。

IT 管理者が [Office での接続エクスペリエンスの使用](/deployoffice/privacy/manage-privacy-controls#policy-setting-for-most-connected-experiences)を無効にした場合、オプションの接続エクスペリエンスのみをオフにした場合と同じ効果がアドインに与えられます。
...
> [!NOTE]
> アドインが初めて読み込まれると、ドメインを信頼するように求めるセキュリティ プロンプトが表示される場合があります。 これは、アドインのドメイン ホストが Exchange オンプレミスまたはOffice Online Serverのドメインの外部にある場合に発生します。
...
- ユーザーは、AppSource へのアクセスを有効または無効にすることができます。 コンテンツ アドインと作業ウィンドウ アドインの場合、ユーザーは、ホスト Office クライアントの **セキュリティ センター** から信頼されたアドインとカタログへのアクセスを管理します ( **ファイル** > **オプション** > **のセキュリティ センターのセキュリティ センター** > **設定** > **の信頼されたアドイン カタログ** から開きます)。 Outlook では、ユーザーは [アドインの取得] を選択して **アドインを** 管理できます。

  - Outlook on Windows で、リボンから **[その他のアプリ** ] を選択し、[ **アドインの取得**] を選択します。
  - Outlook on Mac で、リボンから省略記号ボタン (`...`) を選択し、[ **アドインの取得**] を選択します。
  - Outlook on the webで、リボンから **[アドインの取得**] を選択します。

  管理者は、 [グループ ポリシーを使用して](/previous-versions/office/office-2013-resource-kit/jj219429(v=office.15)#using-group-policy-to-manage-how-users-can-install-and-use-apps-for-office)このアクセスを管理することもできます。
...
Office アドインの開発者向けの PII 保護ガイドラインをいくつか次に示します。
...
- コンテンツ アドインと作業ウィンドウ アドインは、ブラウザーが既定で使用するものと同じ SSL 設定を前提とし、ほとんどのコンテンツを SSL でのみ配信できるようにします。 Outlook アドインは、すべてのコンテンツが SSL で提供されている必要があります。 開発者は、アドインの HTML ファイルの場所を識別するために、HTTPS を使用する URL をアドイン マニフェストの要素で指定 **\<SourceLocation\>** する必要があります。
...
| 設定名 | 説明 |
|--------------|-------------|
| セキュリティで保護されていないアドインとカタログの許可 | ユーザーはセキュリティで保護されていない Office アドインを実行できます。これは、SSL でセキュリティで保護された (https://) web ページまたはカタログの場所を持ち、ユーザーのインターネット ゾーンに存在しない Office アドインです。 |
| Web アドインのブロック | ユーザーが Web テクノロジを使用する Office アドインを実行できないようにすることができます。 |
| Office ストアのブロック |  ユーザーが Office ストアから Office アドインを取得または実行できないようにすることができます。 |
