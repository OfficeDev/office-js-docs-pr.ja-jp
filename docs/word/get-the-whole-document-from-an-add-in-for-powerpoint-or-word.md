...
次のコードは、**document.getFileAsync()** メソッドを使用して、PowerPoint または Word ドキュメントを **File** オブジェクトとして取得します。次に、結果の **File** オブジェクト、ゼロになったカウンター、および [sliceCount](http://dev.office.com/reference/add-ins/shared/file.slicecount) を匿名オブジェクトにパッケージ化します。次に、このオブジェクトはローカルで定義された `getSlice` 関数に渡されます。 
