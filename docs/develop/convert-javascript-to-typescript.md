...
- [Visual Studio 2019 以降の](https://www.visualstudio.com/vs/)バージョンの開発Office **/SharePointワークロードが** インストールされている場合
...
5. [参照] **タブが** 選択されている場合は、「 **jquery」と入力します。TypeScript.DefinitelyTyped**。 このパッケージをインストールするか、既にインストールされている場合は更新します。 これにより、jQuery TypeScript 定義がプロジェクトに確実に含まれます。 jQuery のパッケージは、Visual Studio によって生成されたファイル **に** packages.config。
