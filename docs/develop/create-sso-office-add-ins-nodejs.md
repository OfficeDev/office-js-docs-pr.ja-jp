...
1. アドインに必要な権限を検索するには、**[アクセス許可を選択]** の検索ボックスを使用します。 以下を選択します。 アドイン自体に必要なのは最初のものだけです。ただし、 `profile` Office アプリケーションがアドイン web アプリケーションへのトークンを取得するには、そのアクセス許可が必要です。
...
1. `TODO 1` を次のコードに置き換えます。 このコードの注意点は次のとおりです。
    ...
    - この `allowSignInPrompt` オプションを true に設定すると、現在 office にサインインしているユーザーがいない場合、office はポップアッププロンプトを開きます。
    - この `allowConsentPrompt` オプションを true に設定すると、ユーザーがアドインにユーザーの AAD プロファイルへのアクセスを許可していない場合、Office は同意プロンプトを開きます同意。 (このプロンプトでは、ユーザーは、Microsoft Graph の範囲ではなく、ユーザーの AAD プロファイルに同意することのみが許可されます)。
