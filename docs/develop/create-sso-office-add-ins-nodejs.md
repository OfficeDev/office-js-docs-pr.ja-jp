...
1. コンソールで `npm install` を入力して、package.json ファイル内のアイテム化されたすべての依存関係をインストールします。

1. コンソールで `npm run build` を入力して、プロジェクトをビルドします。
...
1. `client_id` プロパティの場合は、アドインの登録時に保存したアプリケーション ID でプレースホルダーの `{client GUID}` を置き換えます。完了すると、単一引用符で囲まれた GUID のみになります。"{}" 文字は取り去る必要があります。

1. `client_secret` プロパティのプレースホルダー `{client secret}` は、アドインの登録時に保存したアプリケーション シークレットに置き換えます。
...
1. `TODO4` を次のコードに置き換えます。 エラー 13003 は、ユーザーが職場または学校アカウント、または Microsoft アカウントのいずれでもないアカウントでログインしている場合に発生します。 サインアウト後にサポートされているアカウントの種類でもう一度サインインするよう、ユーザーに求めます。
    ...
    > [!NOTE]
    > エラー 13004 は開発時にのみ発生するため、このメソッドでは処理しません。 これはランタイム コードで修正できるものではなく、エンド ユーザーに報告しても意味がありません。
...
1. `TODO15` を次のコードに置き換えます (*前の手順で追加したコードの最後にある右波かっこのすぐ下*)。このコードの注意点は次のとおりです。
...
1. `TODO16` を次のコードに置き換えます (*前の手順で追加したコードの最後にある右波かっこのすぐ下*)。
...
1. \src\auth.ts ファイルを開きます。`AuthModule` クラスに、次に示すメソッドを追加します。このコードについては、次の点に注意してください。
    ...
    * resource パラメーターは省略可能です。 [Secure Token Service (STS)](/previous-versions/windows-identity-foundation/ee748490(v=msdn.10)) が AAD V 2.0 エンドポイントの場合は使用しないでください。 V 2.0 エンドポイントでは scopes から resource を推測し、resource が HTTP 要求で送信される場合に、エラーが返されます。
...
1. `TODO3` を次のコードに置き換えます。このコードの注意点は次のとおりです。
...
1. `TODO4` を次に示すコードに置き換えます。このコードでは、HTTP 要求を STS のトークン エンドポイントに送信します。
...
1. `TODO5` を次のコードに置き換えます。 例外をスローしても、即時の "500 Internal Server Error" がクライアントに送信*されない*点に注意してください。 server.js ファイルでコードを呼び出すことで、この例外をキャッチしてから、その例外をクライアントに送信するエラー メッセージに変換します。
...
1. `TODO6` を次に示すコードに置き換えます。このコードはリソースへのアクセス トークンを永続化して、有効期限になると、そのアクセス トークンを返します。コードを呼び出すことで、期限切れになっていないリソースへのアクセス トークンが再使用されるため、STS への不要な呼び出しを回避できます。この動作のしくみは、次のセクションで説明します。
...
1. 次に示すメソッドをファイルの末尾に追加します。このメソッドにより、アドインのホーム ページを提供します。アドイン マニフェストで、ホーム ページの URL を指定します。
...
1. ファイルの末尾に次のメソッドを追加します。このメソッドが、`values` API に対するすべての要求を処理します。
...
1. `TODO8` を次のコードに置き換えます。このコードについては、次の点に注意してください。
...
1. `TODO9` を次のコードに置き換えます。このコードについては、次の点に注意してください。
...
1. ファイル src\odata-helper.ts を開きます。 このファイルは、ほとんど完成しています。 要求の「終了」イベントを処理するコールバックの本文が欠落しています。 `TODO` を次のコードに置き換えます。 このコードの注意点は次のとおりです。
    ...
    * OData エンドポイントからの応答は、エラーである可能性があります。たとえば、エンドポイントがアクセス トークンを必要としていて、そのトークンが無効または有効期限切れの場合は 401 になります。 ただし、エラー メッセージは ** の呼び出しでのエラーではなく`https.get`であるため、`on('error', reject)` の最後の行 `https.get` はトリガーされません。 そのため、コードでは、成功 (200) とエラー メッセージを区別して、要求された OData またはエラー情報のどちらかを含む JSON オブジェクトを呼び出し元に送信します。
...
1. `TODO2` を次のコードに置き換えます。このコードの注意点は次のとおりです。
    ...
    * Promise オブジェクトは解決されます。拒否されません。 `https.get` は、Web サービスがサーバー間の OData エンドポイントを呼び出すときに実行されます。 ただし、その呼び出しは、クライアントから Web サービスの Web API への呼び出しのコンテキストで行われます。 クライアントから Web サービスへの「外部」の要求は、「内部」の要求が拒否されると完了できなくなります。 さらに、`Error` の呼び出し元が OData エンドポイントからクライアントにエラーを中継する必要がある場合は、カスタムの `http.get` オブジェクトを含む要求も解決する必要があります。
...
1. ネットワーク共有を作成するか、[フォルダーをネットワークに共有します](/previous-versions/windows/it-pro/windows-server-2008-R2-and-2008/cc770880(v=ws.11))。

1. プロジェクトのルートから、Office-Add-in-NodeJS-SSO.xml マニフェスト ファイルのコピーを共有フォルダーに配置します。

1. PowerPoint を起動して、ドキュメントを開きます。

1. **[ファイル]** タブを選択して、**[オプション]** を選択します。

1. [**セキュリティ センター**] を選択し、[**セキュリティ センターの設定**] ボタンを選択します。

1. **[信頼されているアドイン カタログ]** を選択します。

1. **[カタログの URL]** フィールドに、Office-Add-in-NodeJS-SSO.xml があるフォルダー共有へのネットワーク パスを入力して、**[カタログの追加]** を選択します。

1. **[メニューに表示する]** チェック ボックスをオンにして、**[OK]** を選択します。

1. これらの設定は Microsoft Office を次回起動したときに適用されることを示すメッセージが表示されます。PowerPoint を終了します。
...
1. Visual Studio Code を使用していない場合:
   ...
   1. ターミナルで、「**npm run build**」と入力します。
   1. 2 番目のノード ターミナルを開いて、プロジェクトのルート フォルダーに移動します。
   1. ターミナルで、「**npm run start**」と入力します。

1. VS Code を使用している場合:
   1. VS Code でプロジェクトを開きます。
   1. CTRL + SHIFT + B を押して、プロジェクトをビルドします。
   1. **F5** を押して、デバッグ セッションでプロジェクトを実行します。
...
1. **[開発]** タブがリボンに表示されていない場合、次の手順で有効にします。
   ...
   1. チェック ボックスをオンにし、**[リボンのユーザー設定]** ページの右にあるコントロール名のツリーで **[開発]** を有効にします。
   1. **[OK]** を押します。

1. PowerPoint の **[開発]** タブで、**[個人用アドイン]** を選択します。

1. **[共有フォルダー]** タブを選択します。

1. **[SSO NodeJS Sample]** を選択して、**[OK]** を選択します。

1. **[ホーム]** リボンに、**[SSO NodeJS]** という新しいグループが表示され、**[アドインの表示]** というラベルの付いたボタンとアイコンが含まれています。

## <a name="test-the-add-in"></a>アドインをテストする

1. 結果を確認できるように、OneDrive 内にファイルがいくつかあることを確認します。

1. **[アドインの表示]** ボタンをクリックして、アドインを開きます。

1. [ようこそ] ページでアドインが開きます。**[OneDrive からファイルを取得]** ボタンをクリックします。

1. Office にサインインしている場合は、このボタンの下に OneDrive にあるファイルとフォルダーのリストが表示されます。これは、初回実行時には 15 秒以上かかることがあります。

1. Office にサインインしていない場合は、ポップアップが表示され、サインインするように求められます。 サインインが完了すると、数秒後にファイルとフォルダーの一覧が表示されます。 *2 回目はボタンを押す必要はありません。*
