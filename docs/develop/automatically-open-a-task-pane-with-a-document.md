...
ユーザーがアドインコマンドを使用しないアドインを起動すると (たとえば、Office 2013 で実行されているアドイン)、そのアドインはドキュメントに挿入され、そのドキュメントに保存されます。その結果、他のユーザーがドキュメントを開くと、アドインをインストールするように求めるメッセージが表示され、作業ウィンドウが開きます。このモデルの課題は、多くの場合、ユーザーがドキュメントでアドインを永続化したくないということです。たとえば、Word 文書で辞書アドインを使用する学生は、classmates や教師が文書を開くときに、そのアドインをインストールするように求めるメッセージを表示しないようにすることができます。
...
- Autoopen 機能を使用して、アドインの使用を意図的に向上させないでください。特定のドキュメントでアドインを自動的に開くことが適切でない場合は、この機能によってユーザーに迷惑を持たせる可能性があります。
