...
このクイック スタートでは、 **Office アドイン用の Yeoman ジェネレーター** が提供する証明書を使用できます。 このジェネレーターは、このクイック スタートの**前提条件**の一部としてグローバルにインストールされているため、グローバル インストールの場所からアプリ フォルダーに証明書をコピーするだけで済みます。 次の手順では、このプロセスを実行する方法を示しています。
