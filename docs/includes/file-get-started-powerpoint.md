...
2. **Visual C#** または **Visual Basic の**[プロジェクトタイプの一覧で **Office、SharePoint**を展開し、 **アドイン**を選択し、** PowerPoint Webのアドイン** をプロジェクトタイプとして選択します。 
...
4. `DefaultValue`  要素の `Description`   属性にはプレースホルダーがあります。 これを**PowerPoint の作業ウィンドウ アドイン** に置き換えます。
...
3. Yeoman ジェネレータを使用して PowerPoint アドインのプロジェクトを作成してください。次のコマンドを実行し、プロンプトに次のように応答します。
...
3. 要素`DefaultValue` の`Description`  属性にはプレースホルダーが含まれています。 これは、**PowerPoint 用の作業ウィンドウ アドイン** に置き換えてください。
