...
3. ボタンを格納している `div` の下に、次のマークアップを追加します。`sort-table`
...
5. ボタンにクリック ハンドラーを割り当てる行の下に、次のコードを追加します。`sort-chart`

    ```js
    $('#create-chart').click(createChart);
    ```

6. 関数の下に、次の関数を追加します。`sortTable`
...
7. を次のコードに置き換えます。ヘッダー行を除外するために、このコードでは、`getRange` メソッドではなく `Table.getDataBodyRange` メソッドを使用してグラフを作成するデータの範囲を取得しています。`TODO1`
...
8. を次のコードに置き換えます。`TODO2` 次のパラメーターに注意してください。
...
9. を次のコードに置き換えます。`TODO3` このコードのほとんどの部分は、わかりやすく説明不要なものです。 注意:
   - メソッドへのパラメーターでは、グラフを収容するワークシート領域の左上と右下のセルを指定します。`setPosition` Excel では、所定の空間内でグラフの外観を整えるために線幅などを調整できます。
