---
title: アプリの開発者ツールを使用してアドインをデバッグWindows
description: アプリの開発者ツールを使用Microsoft EdgeアドインをデバッグWindows
ms.date: 10/05/2021
...
ms.locfileid: "60138811"
---
# <a name="debug-add-ins-using-developer-tools-on-windows"></a>アプリの開発者ツールを使用してアドインをデバッグWindows

IDEs の外部には、アドインのデバッグに役立つ開発者ツールWindows 10 11. これは、IDE の外部でアドインを実行しているときに問題を調査する必要がある場合に便利です。

使用するツールは、アドインが Microsoft Edge または Internet Explorer のどちらで実行されているかによって異なります。 これは、コンピューターにインストールされているWindowsのバージョンOfficeによって決まります。 開発用コンピューターで使用されているブラウザーを確認するには、「[Office アドインによって使用されるブラウザー](../concepts/browsers-used-by-office-web-add-ins.md)」を参照してください。
...
アドインが Internet Explorer で実行されている場合は、Windows の F12 開発者ツールからデバッガーを使用してアドインをテストできます。 アドインの実行後、F12 開発者ツールを起動できます。 F12 ツールは個別のウィンドウに表示され、Visual Studio を使用しません。

> [!NOTE]
> デバッガーは、デバッガー、11、および Windows 10 の F12 開発者ツールの一部Internet Explorer。 Windows の以前のバージョンにはデバッガーは含まれません。 
